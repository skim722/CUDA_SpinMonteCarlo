...
void try_move(double * try_spin, double delta_max){
  ...
  double delta [DIMENSIONS];
    ...
    s=mynorm(try_spin);
    for (int l=0;l<DIMENSIONS;l++) try_spin[l]/=s;

}

bool MetropolisStep (double delta_max) {
     ...
     if (std_rand() < ratio) {
          for (int l=0;l<DIMENSIONS;l++) spins[i][j][k][l]=-1*spins[i][j][k][l];
                                           //try_move(spins[i][j][k],delta_max);
          return true;
     } else return false;
}
...
double oneMonteCarloStepPerSpin (double delta_max ) {
     int accepts = 0;
     for (int i = 0; i < N; i++)
          if (MetropolisStep(delta_max))
               ++accepts;
